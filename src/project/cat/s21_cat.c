...
int main(int argc, char *argv[]) {
    ...
    if (flags.uncorrect) fprintf(stderr, "Uncorrect flag\n");
    return 0;
}
struct flags parsing_flags(struct flags flags, char* word, unsigned int k) {
    int l = strlen(word);
    for (int j = k; j <  l && !flags.uncorrect; j++) {
        if (word[j] == 'b') {flags.b = 1; continue;}
        if (word[j] == 'e') {flags.e = 1; flags.v = 1; continue;}
        if (word[j] == 'n') {flags.n = 1; continue;}
        if (word[j] == 's') {flags.s = 1; continue;}
        if (word[j] == 't') {flags.t = 1; flags.v = 1; continue;}
        if (word[j] == 'v') {flags.v = 1; continue;}
        if (word[j] == 'T') {flags.T = 1; continue;}
        if (word[j] == 'E') {flags.E = 1; continue;}
        flags.uncorrect = 1;
    }
    return flags;
}
void open_file(char* name, struct flags flags) {
    ...
    if (!file) { fprintf(stderr, "Error file\n");
    } else {
        ...
        while (fgets(arr, N, file)) {
            inner_counter++;
            if (inner_counter == counter_strings) counter_strings = 1;
            if (flags.b) {
                if (!collapse || strlen(arr) > 1 || (strlen(arr) == 1 && arr[0] != 10))
                    print_str_with_b(arr, &c);
                if (!flags.s) printf_str(arr, flags, counter_strings);
                else
...
void printf_str(char arr[N], struct flags flags, int counter_strings) {
    for (unsigned int i = 0; (counter_strings != 1 && arr[i] != '\n') ||
     (counter_strings == 1 && arr[i]); i++) {
        if (flags.v && (arr[i] >= 0 && arr[i] < 32 && arr[i] != 9 && arr[i] != 10)) {
            printf("^%c", arr[i] + 64);
