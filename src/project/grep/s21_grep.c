...
struct flags parsing_string(char *word, struct flags flags, char *array_of_words[N],
                            char *array_of_files[N]);
void standart_situation(struct flags* flags, char *array_of_words[N], char *array_of_files[N]);
...
void case_proccessing(char *file, char *word, int *stop_flag, struct flags* flags);
void proccessing(struct flags* flags, char *array_of_words[N], char *array_of_files[N]);
...
struct flags parsing_flags(struct flags flags, char* flag) {
    if (flag[1] == 'e') { flags.e = 1; flags.counter_e++; flags.input_e = 1; }
    else if (flag[1] == 'i') flags.i = 1;
    else if (flag[1] == 'v') flags.v = 1;
    else if (flag[1] == 'c') flags.c = 1;
    else if (flag[1] == 'l') flags.l = 1;
    else if (flag[1] == 'n') flags.n = 1;
    else
        flags.uncorrect = 1;
    return flags;
}
...
char flags_to_symbol(struct flags *flags) {
    char symbol = '0';
    if (flags->e) symbol = 'e';
    else if (flags->i) symbol = 'i';
    else if (flags->v) symbol = 'v';
    else if (flags->c) symbol = 'c';
    else if (flags->l) symbol = 'l';
    else if (flags->n) symbol = 'n';
    else if (!flags->uncorrect) symbol = 'e';
    return symbol;
}
void printing_strings(char buf_str[N]) {
    int c = 0;
